...
class LauncherWindow:
    ...
    def _build_titlebar(self):
        ...
        for color in ["#ff5f57", "#febc2e", "#28c840"]:
            tk.Label(bar, bg=color, width=2,
                     relief="flat").pack(
                side="left",
                padx=(8 if color == "#ff5f57" else 4, 0),
                pady=10)
        tk.Label(bar,
                 text="Advanced Password Cracker v2.0"
                      " — Main Launcher",
    ...
    def _build_header(self):
        ...
        tk.Label(frame,
                 text="Ethical security testing toolkit"
                      "  ·  For authorized use only",
                 bg=BG_DARK, fg=FG_MUTED,
                 font=("Courier", 9)).pack(pady=(4, 0))
        tk.Frame(self.root, bg=BORDER,
    ...
    def _build_module_grid(self):
        ...
        # ── 8 modules now including AI Training ──────
        modules = [
            ("Hash Cracker",
        ...
        accents = [
            "#1e3a4a", "#2a1e3a", "#1e3a2a",
            "#3a2a1e", "#1e2a3a", "#2a3a1e",
            "#3a1e2a", "#1e1e3a",
        ]
    ...
    def _open_hash_cracker(self):
        from ui.hash_cracker_window import (
            HashCrackerWindow)
        HashCrackerWindow(self.root)

    def _open_hybrid(self):
        from ui.hybrid_attack_window import (
            HybridAttackWindow)
        HybridAttackWindow(self.root)

    def _open_wordlist(self):
        from ui.wordlist_generator_window import (
            WordlistGeneratorWindow)
        WordlistGeneratorWindow(self.root)

    def _open_ai_training(self):             # ← NEW
        from ui.ai_training_window import (
            AITrainingWindow)
...
def _coming_soon(parent, name):
    ...
    tk.Label(win, text=name,
             ...
             pady=(30, 8))
    tk.Label(win,
             text="Coming soon — building next!",
             bg=BG_DARK, fg=FG_MUTED,
             font=("Courier", 10)).pack()
